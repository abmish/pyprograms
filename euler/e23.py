...
MAXIMUM = 28124
primelib._refresh(MAXIMUM/2)

abundants = [num for num in xrange(1, MAXIMUM) if sum(primelib.all_factors(num)) > num*2]
...
for num in xrange(1, MAXIMUM):
    ...
    for x in abundants:
        if x > num:
            break
        if abundants_dict.get(num - x):
            abundants_sum = 1
            break

    if not abundants_sum:
        total = total + num
